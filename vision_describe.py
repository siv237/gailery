...
SYSTEM_PROMPT_WITH_NAMES = """Ты описатель фотографий. Описывай только то что реально видно — без фантазии.
...
def describe_one(img_b64, photo_path, agent_context=""):
    ...
    has_names = bool(agent_context and "На фото" in agent_context and "без имени" not in agent_context.split("На фото")[1].split("\n")[0]) if agent_context else False
    if agent_context and "На фото" in agent_context:
        first_line = agent_context.split("\n")[0]
        has_names = "— " in first_line or " — " in first_line
    sys_prompt = get_system_prompt(has_names)
    _ch = ""
    try:
    ...
    if has_names:
        ...
        user_text = "Опиши что видно на фото. Перечисли видимые предметы, мебель, одежду, детали. Не пиши про то чего не видно. Начинай сразу с описания, без скобок, без заголовка. Не короче 4 предложений. Не придумывай имена людей — описывай по внешности (женщина/мужчина/девочка/мальчик). Возраст не угадывай."
    if agent_context:
        user_text += "\n\n" + agent_context
    data = {
        "messages": [
